...
## Matrix multiplication {#sec-matrix-mult}
...
and define the product of matrices $\mathbf{A}$ and $\mathbf{B}$ as the matrix $\mathbf{C} = \mathbf{A}\mathbf{B}$ that has entries $c_{ij}$ equal to the sum of the component-wise product of the $i$th row of $\mathbf{A}$ with the $j$th column of $\mathbf{B}$. 

We can define matrix multipicaiton $\mathbf{A}\mathbf{B}$ for any two matrices, say
...
Using mathematical notation $\mathbf{A}\mathbf{B}$ looks like this:
...
Note this definition implies that the multiplication $\mathbf{A}\mathbf{B}$ is only possible when the number of rows of $\mathbf{A}$ matches the number of columns of $\mathbf{B}$. When this happens, we say the two matrices are *conformable*. 

So, how does this definition of matrix multiplication help solve systems of equations? 

To understand this, consider the following system of equations with unknowns $x_1, \dots x_n$:
...
We can represent this as matrix multiplication by defining the following matrices,
...
If you define $\mathbf{I}$ as a matrix with the same number of rows and columns (referred to as a square matrix) with 0s everywhere except the diagonal,
...
To define distance, we introduce another linear algebra concept: the *norm*. A point in two dimensions can be represented in polar coordinates as:
...
To define distance, suppose we have two points in two-dimensional space: $\mathbf{x}_1$ and $\mathbf{x}_2$. We can define how similar they are by simply using Euclidean distance:
...
The reason we introduced the norm is that this distance is the size of the vector between the two points, and this can be extrapolated to any dimension. The distance between two points, regardless of the dimensions, is defined as the norm of the difference

$$
\| \mathbf{x}_1 - \mathbf{x}_2\|.
$$

If we use the digit data, the distance between the first and second observation will be computed using all 784 features:

$$
\| \mathbf{x}_1 - \mathbf{x}_2 \| = \sqrt{ \sum_{j=1}^{784} (x_{1j}-x_{2j })^2 }
$$
...
In R, the function `crossprod(x)` is convenient for computing norms. It multiplies `t(x)` by `x`. The following is the same as `sum((x_1 - x_2)^2)`:

```{r}
crossprod(x_1 - x_2)
```

Note that `crossprod` takes a matrix as the first argument. As a result, the vectors used here are being coerced into single-column matrices. Also, note that `crossprod(x, y)` multiplies `t(x)` by `y`.
...
This is convenient because there are several machine learning-related functions in R that take objects of class `dist` as input. 
...
Note that the diagonal is omitted because all self-distances are zero, and the upper triangle is also excluded due to the symmetry of the distance matrix, which follows from the fact that distance is commutative. That is, the distance from `x` to `y` is equivalent to the distance from `y` to `x`.
 
To access the entries using row and column indices, we need to coerce it into a matrix. 

The `image` function allows us to quickly see an image of the distances between observations. As an example, we compute the distance between each of the first 300 observations like this:

```{r}
d <- dist(x[1:300,])
```

To see an image, we simply convert `d` to a matrix and send it to the function: `image(as.matrix(d))`.

If we order this distance by the labels, we can see yellowish squares near the diagonal. This is because observations from the same digits tend to be closer than observations from different digits:

```{r diatance-image-ordered, fig.width = 4, fig.height = 4, out.width="50%", echo=-1}
rafalib::mypar()
...
We can think of all predictors $(x_{i1}, \dots, x_{ip})^\top$ for all observations $i=1,\dots,n$ as $n$ $p$-dimensional points. A *space* can be thought of as the collection of all possible points that should be considered for the data analysis in question. This includes points we could see, but have not been observed yet. In the case of the handwritten digits, we can think of the predictor space as any point $(x_{1}, \dots, x_{p})^\top$ as long as each entry $x_i, \, i = 1, \dots, p$ is between 0 and 255.

Some Machine Learning algorithms also define subspaces. A commonly defined subspace in machine learning is a _neighborhood_ composed of points that are close to a predetermined *center*. We do this by selecting a center $\mathbf{x}_0$, a minimum distance $r$, and defining the subspace as the collection of points $\mathbf{x}$ that satisfy:

$$
\| \mathbf{x} - \mathbf{x}_0 \| \leq r.
$$

We can think of this subspace as a multidimensional sphere since every point is within the same maximum distance away from the center.

Other machine learning algorithms partition the predictor space into non-overlapping regions and then make different predictions for each region using the data in that region. We will learn about these in [@sec-trees].
...
Generate a boxplot showing the distances from the second row of `d` to all other observations, stratified by digit label. Do not include the distance to itself, which we know is a distance of 0. Based on the boxplot, can you predict what digit is represented by the second row of `x`?

4\. Use the `apply` function and matrix algebra to compute the distance between the fourth digit `mnist$train$images[4,]` and all other digits represented in `mnist$train$images`. Then generate a boxplot as in exercise 3 and predict what digit is in the fourth row.

5\. Compute the distance between each feature and the feature corresponding to the middle pixel (row 14, column 14). Display the resulting distances as a 28 $\times$ 28 image, with color indicating the distance of each pixel position.
