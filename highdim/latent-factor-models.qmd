...
and then, for example,  compare the residuals for *The Godfather* with three other films:
...
We start with a simple simulated example of ratings $Y_{ij}$ for six movies and 120 users. To illustrate how factor analysis uncovers hidden structure, we don't show the data-generating code. For simplicity, we assume ratings have been adjusted for movie and user effects (as described in [@sec-regularization]), and we store the residuals in the object `y`:
...
One approach is to use our knowledge of movie genres to define a factor that distinguishes between mob and romance movies:
...
We include `-1` in the formula because the residuals were constructed to have a mean of 0, so no intercept is needed. 

Each call to `lm` returns an estimated coefficient representing the difference in average ratings between mob and romance movies. A positive value indicates a preference for mob movies, a negative value indicates a preference for romance movies, and values near 0 suggest no strong preference. 

Collecting these across users gives us `p`, a vector with one entry per user. The histogram below shows users cluster into these three types: 
...
Using matrix representation, we can rewrite the above expression like this:

$$
\mathbf{Y} \approx \mathbf{P}\mathbf{Q}^\top
$$
with $\mathbf{Y}$ an $I\times J$ matrix with entries $Y_{ij}$, $\mathbf{P}$ a $I\times K$ matrix with entries $p_{ik}$, and $\mathbf{Q}$ a $J\times K$ matrix with entries $q_{jk}$, where $K$ is the number of latent factors (here, $K = 2$).

This analysis provides insights into the process of generating our data since $\mathbf{P}$ contains user-specific parameters and $\mathbf{Q}$ contains movie-specific parameters. The approach is often referred to as *matrix factorization* because the rating matrix has been *factorized* into two lower-dimensional, interpretable matrices. 
...
$$
\begin{aligned}
Y_{ij} &= \sum_{k=1}^K p_{ik}q_{jk} + \varepsilon_{ij}, \quad i=1,\dots,I,\; j=1,\dots,J,\\
\text{or equivalently}\quad \mathbf{Y} &= \mathbf{P}\mathbf{Q}^\top + \boldsymbol{\varepsilon}.
\end{aligned}
$$
with the constraint 
...
Another way to see this connection is through optimization. PCA can be formulated as the solution to a least squares problem: among all possible $K$-dimensional projections of the data, PCA finds the one that minimizes the reconstruction error. That is, of any $K$ latent factors that could describe the data, the first $K$ PCs minimize the sum of squared differences between the original data $\mathbf{Y}$ and its approximation $\mathbf{P}\mathbf{Q}^\top$. In this sense, PCA provides the best rank-$K$ approximation of $\mathbf{Y}$ in the least squares sense. 
...
:::{.callout-note}
Even with the orthogonality constraint, the PCA solution is not completely unique.  As pointed out in @sec-dimension-reduction, there remains a *sign indeterminacy*: we can flip the sign of any column in $\mathbf{P}$ and the corresponding column in $\mathbf{Q}$ (multiply one by $-1$ and the other by $-1$) without changing the product $\mathbf{P}\mathbf{Q}^\top$.  
Thus, the factorization is identifiable only up to these sign changes.
:::

## Case study: recommendation systems

We now move back from the small simulated example to the real Movielens dataset. In the previous section, we used a toy setting to illustrate how latent factors can capture patterns such as "mob vs. romance" or "Al Pacino vs. not Al Pacino". Here, we show that a similar latent structure is present in the real movie ratings and that we can exploit it to further improve predictions.
...
This motivates extending our earlier model with latent factors that capture similarities between movies and users. This approach is a simplified version used by the Netflix Prize winners, who combined latent factor models with the regularization methods from @sec-regularization.

We use a latent factor model that extends the model from @sec-regularization to include factors capturing similarities between movies:
$$
...
$$
\hat{y}_{ij} = \hat{\mu} + \hat{\alpha}_i + \hat{\beta}_j + \hat{p}_i^\top \hat{q}_j
$$
...
Here, $N$ is the number of observed ratings, $I$ is the number of users, and $J$ is the number of movies. The vectors $\boldsymbol{\alpha}$ and $\boldsymbol{\beta}$ are the user and movie effects, $\boldsymbol{\alpha} = (\alpha_1,\dots,\alpha_I)^\top$ and $\boldsymbol{\beta} = (\beta_1,\dots,\beta_J)^\top$. The vectors $\mathbf{p}_k = (p_{1k}, \dots, p_{Ik})^\top$ and $\mathbf{q}_k = (q_{1k}, \dots, q_{Jk})^\top$ are the $k$-th latent factor components. Recall that for any vector $\mathbf{x} = (x_1,\dots x_n)^\top$, $\|\mathbf{x}\|^2$ denotes the sum of squares, $\sum_{i=1}^n x_i^2$.
...
The **behdslabs** package provides the `fit_recommender_model` function, which implements a latent factor model with regularization:

```{r}
#| warning: false
fit <- with(train_set, 
  fit_recommender_model(rating, userId, movieId, reltol=1e-6))
```

You can learn more about the function by running `?fit_recommender_model` or by examining the source code.
...
i <- test_set$userId
j <- test_set$movieId
res <- test_set$rating - with(fit, mu + a[i] + b[j] + rowSums(p[i,]*q[j,]))
rmse(res)
```
...
Looking at the movies with the most extreme factor values (red) provides a clear interpretation for the factors. 

The first factor separates iconic dark classics

```{r}
names(sort(factors[,1], decreasing = TRUE)[1:5]) |> cat(sep = "\n")
```

from Hollywood blockbusters

```{r}
names(sort(factors[,1])[1:5]) |> cat(sep = "\n")
```

The second factor separates weird cult movies

```{r}
names(sort(factors[,2], decreasing = TRUE)[1:5]) |> cat(sep = "\n")
```

from big-budget epic historical and fantasy sagas

```{r}
names(sort(factors[,2])[1:5]) |> cat(sep = "\n")
```
...
In the next exercises, we use the singular value decomposition (SVD) to estimate latent factors in a school-performance example.
...
$$
with $\mu_{ij}$ the expected grade for student $i$ in class $j$ and $\varepsilon_{ij}$ random variation due to random chance in answering exam questions (guessing or dumb mistakes, for example). We assume the $\varepsilon_{ij}$ are independent from each other. We will simulate data with:
...
m <- m[order(rowMeans(m)),] %x% matrix(rep(1,k), nrow = 1)
y <- m + matrix(rnorm(n * k * 3, 0, 3), n, k * 3)
colnames(y) <- paste(rep(c("Math", "Science", "Arts"), each = k), 1:k)
```
...
axis(2, at = 1:ncol(y), labels = rev(colnames(y)), las = 2, cex.axis = 0.5)
```
...
Which option best describes the correlation pattern?
   ...
   d. There is a correlation among all tests, with stronger correlation within each subject and stronger correlation between Math and Science than between either and Arts.
...
Compute the SVD and plot the proportion of variance explained by each PC:

```{r}
#| eval: false
s <- svd(y)
plot(1:ncol(y), s$d^2/sum(s$d^2), xlab = "PC", ylab = "Var. explained")
```
...
How would you interpret the first PC in plain language? (Remember the sign is arbitrary, so high PC1 can correspond to a strong student or a weak student, depending on the sign.)
...
and repeat the exploratory plots generated in exercise 13. Has most of the correlation structure disappeared, or is there still a visible within-subject pattern?
...
17\. Based on the patterns seen in exercise 16, how would you interpret the values $u_{i1}, \dots, u_{ip}$ for student $i$:
