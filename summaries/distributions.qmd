...
heights |> filter(sex == "Male") |> ggplot(aes(x = height)) + 
  geom_histogram(breaks = c(50, 55, 60:80, 85), color = "black")
```
...
The y-axis is scaled so the total area under the curve equals 1. Probabilities are represented by areas under the curve. For example, the proportion of values between 65 and 68 is the area over that interval:
...
This proportion is about `r round(mean(dplyr::between(heights$height[heights$sex=="Male"], 65, 68)), 2)`, meaning roughly `r noquote(paste0(round(mean(dplyr::between(heights$height[heights$sex=="Male"], 65, 68)), 2)*100, '%'))` of male heights fall in this range.
