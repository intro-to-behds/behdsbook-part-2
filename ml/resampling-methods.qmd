...
In this chapter, we introduce resampling, one of the most important ideas in machine learning. Here, we focus on the conceptual and mathematical aspects. To motivate the concept, we will use the two-predictor gesture data presented in [@sec-two-or-seven] and introduce k-nearest neighbors (kNN) to demonstrate the ideas. We will describe how to implement resampling methods in practice with the **caret** package later in [@sec-caret]. 
...
Due to the connection we described earlier between conditional expectations and conditional probabilities, this gives us $\hat{p}(\mathbf{x}_0)$, just like the bin smoother gave us an estimate of a trend. As with bin smoothers, we can control the flexibility of our estimate through a parameter, in this case, the number of neighbors $k$: larger $k$s result in smoother estimates, while smaller $k$s result in more flexible and wiggly estimates.
...
library(behdslabs)
library(caret)
knn_fit <- knn3(y ~ ., data = gesture_swipe_data$train, k = 5)
```
...
```{r}
y_hat_knn <- predict(knn_fit, gesture_swipe_data$test, type = "class")
```

We see that kNN, with the default parameter, already beats GLM:

```{r}
mean(y_hat_knn == gesture_swipe_data$test$y)
```
...
p2 <- plot_cond_prob(predict(knn_fit, gesture_swipe_data$true_p)[,2]) +
...
```{r}
y_hat_knn <- predict(knn_fit, gesture_swipe_data$train, type = "class")
mean(y_hat_knn == gesture_swipe_data$train$y)
```

This is due to what we call *over-fitting*.

## Over-fitting

With kNN, over-fitting is at its worst when we set $k = 1$. With $k = 1$, the estimate for each $\mathbf{x}$ in the training set is obtained with just the $y$ corresponding to that point. In this case, if the $x_1$ and $x_2$ are unique, we will obtain perfect accuracy in the training set because each point is used to predict itself (if the predictors are not unique and have different outcomes for at least one set of predictors, then it is impossible to predict perfectly).

Here we fit a kNN modelx with $k = 1$ and confirm we get near-perfect accuracy in the training set:

```{r}
knn_fit_1 <- knn3(y ~ ., data = gesture_swipe_data$train, k = 1)
y_hat_knn_1 <- predict(knn_fit_1, gesture_swipe_data$train, type = "class")
mean(y_hat_knn_1 == gesture_swipe_data$train$y)
```

But in the test set, accuracy is actually worse than what we obtained with regression:

```{r}
y_hat_knn_1 <- predict(knn_fit_1, gesture_swipe_data$test, type = "class")
mean(y_hat_knn_1 == gesture_swipe_data$test$y)
```
...
tmp <- gesture_swipe_data$true_p
tmp$knn <- predict(knn_fit_1, newdata = gesture_swipe_data$true_p)[,2]
p1 <- tmp |>
  ggplot() +
  geom_point(data = gesture_swipe_data$train, aes(x_1, x_2, color = y),
...
p2 <- tmp |>
  ggplot() +
  geom_point(data = gesture_swipe_data$test, aes(x_1, x_2, color = y), 
             pch = 21, show.legend = FALSE) +
  scale_fill_gradientn(colors = c("#F8766D","white","#00BFC4")) +
  stat_contour(aes(x_1, x_2, z = knn), breaks = c(0.5), color = "black") +
  ggtitle("Test set")
...
Although not as badly as with $k=1$, we saw that with $k = 5$ was also over-fitted. Hence, we should consider a larger $k$. Let's try, as an example, a much larger number: $k = 401$.

```{r}
knn_fit_401 <- knn3(y ~ ., data = gesture_swipe_data$train, k = 401)
y_hat_knn_401 <- predict(knn_fit_401, gesture_swipe_data$test, type = "class")
mean(y_hat_knn_401 == gesture_swipe_data$test$y)
```

The estimate turns out to be similar to the one obtained with regression:

```{r mnist-27-glm-est, echo = FALSE, fig.asp = 0.5}
fit_lm <- lm(y ~ ., data = mutate(gesture_swipe_data$train, y=y == "swipe_right"))
p_hat <- predict(fit_lm, newdata = gesture_swipe_data$true_p)
p_hat <- scales::squish(p_hat, c(0, 1))
p1 <- plot_cond_prob(p_hat) +
  ggtitle("Regression")

p2 <- plot_cond_prob(predict(knn_fit_401, gesture_swipe_data$true_p)[,2]) +
  ggtitle("kNN-401")
...
accuracy <- sapply(ks, function(k){
  fit <- knn3(y ~ ., data = gesture_swipe_data$train, k = k)
  
  y_hat <- predict(fit, gesture_swipe_data$train, type = "class")
  cm_train <- confusionMatrix(y_hat, gesture_swipe_data$train$y)
  train_error <- cm_train$overall[["Accuracy"]]
  
  y_hat <- predict(fit, gesture_swipe_data$test, type = "class")
  cm_test <- confusionMatrix(y_hat, gesture_swipe_data$test$y)
...
First, note that the estimate obtained from the training set is generally more optimistic, with higher accuracy, than the estimate from the test set, with the gap being larger for smaller values of $k$. This is a classic symptom of *overfitting*.
...
To keep track of how these choices affect performance, we write the predictions produced by a given parameter set for observation $i$ as $\hat{y}_i(\lambda)$ and the corresponding mean squared error as $\mathrm{MSE}(\lambda)$. 

$$
\mathrm{MSE}(\lambda) = \mathrm{E}\left[\left\{\hat{Y}(\lambda) - Y\right\}^2\right],
$$

Our goal is to find the value of $\lambda$ that minimizes $\mathrm{MSE}(\lambda)$.
...
$$
\widehat{\mbox{MSE}}(\lambda) = \frac{1}{N}\sum_{i = 1}^N \left\{\hat{y}_i(\lambda) - y_i\right\}^2
$$
...
Overall, we are provided a dataset (blue), and we need to build an algorithm, using this dataset, that will eventually be used in completely independent datasets (orange) that we might not even see.

```{r, echo = FALSE}
knitr::include_graphics("img/cv-1.png")
```

So to imitate this situation, we start by carving out a piece of our dataset and pretending it is an independent dataset: we divide the dataset into a *training set* (blue) and a *test set* (red). We will train the entirety of our algorithm, including the choice of parameter $\lambda$, exclusively on the training set and use the test set only for evaluation purposes.
...
We will do this many times, ensuring that the estimates of MSE obtained in each dataset are independent of each other. There are several proposed methods for doing this. Here, we describe one of these approaches, K-fold cross-validation, in detail to provide the general idea used in all approaches.
...
We want to generate a dataset that can be thought of as an independent random sample, and do this $B$ times. The K in K-fold cross-validation represents the number of times $B$. In the illustrations below, we show an example that uses $B = 5$.
...
$$
\widehat{\mbox{MSE}}_b(\lambda) = \frac{1}{M}\sum_{i = 1}^M \left(\hat{y}_i^b(\lambda) - y_i^b\right)^2 
$$
...
Now, how do we pick the cross-validation fold? Large values of $B$ are preferable because the training data better imitate the original dataset. However, larger values of $B$ will have much slower computation time: for example, 100-fold cross-validation will be 10 times slower than 10-fold cross-validation. For this reason, the choices of $B = 5$ and $B = 10$ are popular.
...
data.frame(k = ks, naive = accuracy["test",], 
  ...
  mutate(set = factor(set, levels = c("naive", "cv", "boot"),
                      labels = c("Simple", "10-fold", "Bootstrap"))) |>
  ggplot(aes(k, Accuracy, color = set)) + 
  geom_line() 
```
...
6\. Load the `sensor_activity_features` dataset.
...
#| eval: false
set.seed(1995)
indexes <- createResample(gesture_swipe_data$train$y, 10)
```
