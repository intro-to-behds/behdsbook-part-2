...
We introduce the **caret** package, which provides useful functions to facilitate machine learning in R, and we describe it in more detail in [@sec-caret]. For our first example, we use the height data provided by the **behdslabs** package.
...
A standard way of generating the training and test sets is by randomly splitting the data. The __caret__ package includes the function `createDataPartition` that helps us generate indices for randomly splitting the data into training and test sets: 

```{r}
set.seed(2010)
train_ind <- createDataPartition(heights$y, p = 0.5, list = FALSE)
```

The argument `times` is used to define how many random samples of indexes to return and it defaults to 1, the argument `p` is used to define what proportion of the data is represented by the index, and the argument `list` is used to decide if we want the indexes returned as a list or not.

We can use the result of the `createDataPartition` function call to define the training and test sets as follows:

```{r}
train_set <- heights[train_ind, ]
test_set <- heights[-train_ind, ]
```
...
Can we do better? Exploratory data analysis suggests we can because, on average, males are taller than females. But how do we make use of this insight? Let's try another simple approach: predict 1 (`Female`) if height is less than two standard deviations from the average male height:
...
But can we do even better? In the example above, we used a cutoff of 62, but we can examine the accuracy obtained for other cutoffs and then pick the value that provides the best results. But remember, **it is important that we optimize the cutoff using only the training set**: the test set is only for evaluation.

In @sec-cross-validation we will learn that evaluating an algorithm on the training set can lead to _overfitting_, which often results in over-optimistic assessments, but for this simplistic example it is not much of a problem.
...
We see that it is a bit lower than the accuracy observed for the training set. This is because we used the training set to select the cutoff, so there is some overfitting. 
However, our cutoff-based approach is still better than guessing. And by testing on a dataset that we did not train on, we know our improvement is not due to overfitting.
...
We notice an imbalance: too many females are predicted to be male. We are calling almost half of the females male! How can our overall accuracy be so high then? This is because the _prevalence_, defined as the proportion of $Y=1$ in our data, is substantially lower than 0.5. These heights were collected from three data sciences courses, two of which had higher male enrollment:
...
There are several metrics that we can use to evaluate an algorithm in a way that prevalence does not cloud our assessment, and these can all be derived from the confusion matrix. A general improvement in using overall accuracy is to study _sensitivity_ and _specificity_ separately. 
...
*Sensitivity* is quantified by $TP/(TP+FN)$, the proportion of actual positives (the first column total, $TP+FN$) that are called positives ($TP$). This quantity is also referred to as the _true positive rate_ (TPR) or _recall_. 

Specificity is defined as $TN/(TN+FP)$, or the proportion of negatives (the second column total, $FP+TN$) that are called negatives ($TN$). This quantity is also called the true negative rate (TNR). 

There is another way of quantifying *specificity*, which is $TP/(TP+FP)$ or the proportion of outcomes called positives (the first row total, $TP+FP$) that are actually positives ($TP$). This quantity is referred to as _positive predictive value (PPV)_ and also as _precision_. Note that, unlike TPR and TNR, precision depends on prevalence since higher prevalence implies you can get higher precision even when guessing. 
...
Before applying this algorithm to general datasets, we need to ask ourselves if the prevalence in the general population will be the same as in our training dataset.
...
$$
F_1\mbox{-score} = \frac{1}{\frac{1}{2}\left(\frac{1}{\mbox{recall}} + 
...
We now see that we do much better than guessing, with sensitivity and specificity both relatively high.


## ROC and precision-recall curves

When comparing the two methods (guessing versus using a height cutoff), we looked at accuracy and $F_1$. The second method clearly outperformed the first. However, while we considered several cutoffs for the second method, for the first, we only considered one approach: guessing with equal probability. Note that guessing `Male` with higher probability would increase overall accuracy because males are more prevalent in the sample. But, as described above, this would come at the cost of lower sensitivity. The curves in this section illustrate the trade-offs between performance metrics across different cutoffs.
...
We can use similar code to compute these values for our second approach. By plotting both curves together, we are able to compare sensitivity for different values of specificity. Each point in the "guessing" curve is labeled by the probability that an individual is predicted to be male; each point in the "height cutoff" curve is labeled by the cutoff above which individuals are predicted to be male.
...
We see that we obtain higher sensitivity with the cutoff  approach for all values of specificity, which implies it is, in fact, a better method than guessing. Keep in mind that ROC curves for guessing always fall on the identity line. The packages **pROC** and **plotROC** are useful for generating these plots.

ROC curves have a key limitation: neither metric they display depends on prevalence. When prevalence is important, a precision-recall curve is often more appropriate. The concept is similar, but instead of plotting true positive rate against false positive rate, we plot precision against recall.
...
From the plot on the left, we immediately see that the precision is low when randomly guessing, reflecting the low prevalence of females. In the plot on the right, redefining $Y=1$ to mean `Male` increases precision because the positive class is now more common. Note that the ROC curve would remain the same.
...
We can use Bayes rule to compute the precision we expect your algorithm to have in the real population: 
$$
\begin{aligned}
...
Here is a plot of precision as a function of prevalence with TPR and TNR both equal to 99%:
...
Although your algorithm achieves a precision of about 99% on the balanced training data (with 50% prevalence), its precision would drop to roughly 33% when applied to the general population, where the disease prevalence is only 0.5%. A doctor cannot rely on a test for which two-thirds of positive results are false positives. At a fixed specificity, even perfect sensitivity would result in around 33% precision. To make the algorithm clinically useful, you would need to drastically increase the specificity, or equivalently, reduce the false positive rate (FPR).
...
Because we often model $y$ as the outcome of a random process, the value of $\hat y$ that minimizes $(\hat{y} - y)^2$ depends on the particular sample. For this reason, we minimize the expected loss, or mean squared error (MSE):

$$
\text{MSE} \equiv \mathrm{E}[(\hat{Y} - Y)^2 ]
$$

where $Y$ represents a new outcome drawn independently from the same distribution as the training data, and $\hat{Y}$ is the corresponding prediction based on the model fitted to the training sample. The expectation is taken over both the randomness in the new observation $Y$ and the randomness in the training data used to construct $\hat{Y}$.
...
$$
\widehat{\mbox{MSE}} = \frac{1}{N}\sum_{i=1}^N (\hat{y}_i - y_i)^2
$$

with the $\hat{y}_i$ generated completely independently from the $y_i$s. 

However, the estimate $\widehat{\text{MSE}}$ is itself a random variable, since it depends on the particular sample used to compute it. The quantity $\text{MSE}$ refers to the *true* (theoretical) prediction error, defined as an expectation over the data-generating distribution, while $\widehat{\text{MSE}}$ is the corresponding sample-based estimate computed from observed data.

For many machine learning algorithms, it is difficult to derive how well $\widehat{\text{MSE}}$ approximates $\text{MSE}$. In @sec-cross-validation, we introduce cross-validation, a general approach for estimating the MSE more reliably.
...
library(caret)
library(titanic)
titanic <- select(titanic_train, Survived, Pclass, Sex, Age, Fare) |> 
  drop_na() |>
  mutate(Survived = factor(Survived, levels = c(1,0)), 
         Pclass = factor(Pclass), Sex = factor(Sex))
```

Note that we defined the levels to be `c(1,0)` to avoid the default behavior of making 0 (first alphabetically) the first level, given that **caret** defines positives $Y=1$ as the first level.

1\. Split the `titanic` dataset into `train_set` with 80% of the data and `test_set` with the remaining 20%.
...
5\. Now repeat exercise 2, but for different cutoffs:
...
6\. Using the results from exercise 5, find the best cutoff, and compare the resulting decision rule's accuracy to the rule described in exercise 4 (using median `Fare` as cutoff). Report accuracy for each rule on both `train_set` and `test_set`. 
...
9\. Consider `cutoffs <- seq(0.05, 0.95, 0.05)` for this probability and generate an ROC curve based on the test set. Based on this ROC curve and the one computed in exercise 7, which method would you say performs better?
