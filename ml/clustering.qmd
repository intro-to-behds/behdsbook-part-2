...
```{r}
genex <- with(tissue_gene_expression, sweep(x, 2, colMeans(x)))
```
...
The basic idea is straightforward: we create an image of the data matrix using color to represent each value, and we reorder both rows and columns according to the results of a clustering algorithm. This reordering places similar samples and similar genes next to each other, making the structure in the data much easier to see.

We illustrate this approach below using the `tissue_gene_expression` dataset from **behdslabs**.
...
The results show that k-means does a reasonably good job at recovering the tissue structure. For example, clusters 1, 3, 4, 5, and 7 correspond largely to the cerebellum, liver, endometrium, kidney, and placenta, respectively. All hippocampus samples fall into cluster 6, although five cerebellum samples are incorrectly included there. Cluster 2 consists of almost all colon samples, with only one kidney sample misassigned.
