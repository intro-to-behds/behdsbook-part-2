...
The main evidence supporting this conclusion is based on a comparison of the percentages that we can compute using the data presented in the paper: 
...
Using basic combinatorics, covered in most discrete probability textbooks, we can compute the probability of correctly identifying $k$ cups out of 4 by chance. Under random guessing, this probability is

$$
\frac{\binom{4}{k}\binom{4}{4-k}}{\binom{8}{4}}.
$$

To assess how surprising the result is, we compute the probability of getting **3 or more correct** by chance:

$$
\frac{\binom{4}{3}\binom{4}{1}}{\binom{8}{4}} + \frac{\binom{4}{4}\binom{4}{0}}{\binom{8}{4}} = \frac{16}{70} \approx 0.24.
$$

This shows that correctly identifying 3 cups could easily occur by chance and does not provide strong evidence for the claimed ability.

This calculation is exactly what *Fisher’s exact test* performs. In R, we can compute the corresponding p-value with:
...
```{r}
r <- with(totals, (yes_men + yes_women))/sum(totals)
```

We use this to compute what we expect to see by chance:

```{r}
e <- with(totals, data.frame(men = (no_men + yes_men)*c(1 - r, r),
                             women = (no_women + yes_women)*c(1 - r, r),
                             row.names = c("no", "yes")))
```
...
cbind(o, e) %>%
  set_names(c(
    paste("obs", colnames(o)),
    paste("expected", colnames(e))
  ))
```
...
Here, we compute the odds ratio for our funding data and construct a 95% confidence interval:
...
:::{.callout-note}
The log odds ratio is not defined if any of the cells of the two-by-two table are 0. This is because if $a$, $b$, $c$, or $d$ are 0, the $\log(\frac{ad}{bc})$ is either the log of 0 or has a 0 in the denominator. For this situation, it is common practice to avoid 0s by adding 0.5 to each cell. This is referred to as the *Haldane-Anscombe correction* and has been shown, both in practice and theory, to work well. 
:::
...
Note that we can write the probability $p_i = Pr(Y_i = 1)$ as a function of the parameters $\beta_0+\beta_1x_i$ for any $x_i$ by using the inverse of the logit transformation:
...
We can confirm that logistic regression yields the same odds ratio estimate as a calculation from the simple two-by-two table (saved in `o`):
...
The general strategy is to assume $Y_i$ follows a known distribution, such as binomial or exponential, and 
...
SAheart |> 
  ...
  geom_point()
```
The points at the bottom of the plot correspond to ages where the observed proportion of heart disease is 0, which leads to log-odds of $-\infty$. The function `ggplot` has the convention of showing these plots cut off at the bottom of the plot. When the proportion is 1, the log-odds would be $\infty$ and the points would be cut off at the top of the plot.  The points in between suggest a roughly linear relationship between age and log-odds of heart disease, which motivates the logistic regression model:
...
When `type = "response"`, the `predict` function uses the inverse logistic transformation to convert $\hat{\beta}_0 + \hat{\beta}_1 x_i$ to estimated probabilities.
...
But how do we assess uncertainty? Can we construct confidence intervals? Note that counts are small (often under 10), and the sample size per group is only 12, so normal approximations may be unreliable.
...
Note that the relationship between the odds ratio and the p-value is not one-to-one; it depends on the sample size. Therefore, a very small p-value does not necessarily mean a very large odds ratio. Observe what happens to the p-value if we multiply our two-by-two table by 10, which does not change the odds ratio:
...
c.  Because the sums of the rows and columns of the two-by-two table are not fixed, the hypergeometric distribution is not an appropriate assumption for the null hypothesis. For this reason, Fisher's exact test is rarely applicable to observational data.
...
11\. For each sentiment, test the null hypothesis that there is no difference between tweets from Android and iPhone, and report the sentiments that are more likely to come from Android with p-values less than 0.05.

12\. For each sentiment, find the words assigned to that sentiment, keep words that appear at least 25 times, compute the odds ratio for each, and show a barplot for those with odds ratios larger than 2 or smaller than 1/2.

13\.  The `titanic_train` dataset in the **titanic** package has data for 891 passengers that were on the Titanic:
...
```{r}
#| eval: false
fit <- glm(Survived ~ Sex, family = binomial, data = titanic_train)
summary(fit)
```
...
fit <- glm(Survived ~ Sex + Pclass, family = binomial, data = titanic_train)
summary(fit)
```
...
17\. In GLMs, although there is no separate error term $\varepsilon$, we can still define *residuals* by comparing the observed outcome $y_i$ to the fitted value $\hat{y}_i$. However, unlike in standard linear regression, the variance of $Y_i$ is not constant. For instance:
