...
When we are unable to randomly assign each individual to a treatment or control group, confounding becomes particularly prevalent. For instance, consider estimating the effect of eating fast food on life expectancy using data collected from a random sample of people in New York City. Fast food consumers are more likely to be smokers, drinkers, and have lower incomes. Consequently, a naive regression model may lead to an overestimate of the negative health effects of fast food. So, how do we account for confounding in practice? In this chapter, we learn how _multivariable regression_ can help with such situations and can be used to describe how one or more variables affect an outcome variable. We illustrate with a real-world example in which data was used to help pick underappreciated players to improve a resource-limited sports team.

:::{.callout-note}
## Connection to Treatment Effects

In @sec-treatment-effect-models, we introduced multivariable regression in the context of binary covariates (treatment groups and confounders). Here, we extend that framework to continuous predictors.
:::
...
5. *Base on Balls (BB)* – The pitcher throws poorly, and the batter is allowed to go to first base to penalize the pitcher.
...
Historically, the *batting average* (AVG) has been considered the most important offensive statistic. To define this average, we divide the total number of hits (H) by the total number of *at-bats* (AB), defined as the number of times you either get a hit or make an out; BB are excluded. Today, this success rate ranges from 20% to 38%. 

```{r, echo = FALSE}
knitr::include_graphics(file.path(img_path,"JumboTron.png"))
```

One of Bill James' first important insights is that the batting average ignores BB, but a BB is a success. Instead of batting average, James proposed the use of the *on-base percentage* (OBP), which he defined as (H+BB)/PA, or simply the proportion of plate appearances that don't result in an out, a very intuitive measure. He noted that a player who accumulates many more BB than the average player might go unrecognized if the batter does not excel in batting average. 

But is this player not helping produce runs? While the highest AVG receives the *batting title* award, no award is given to the player with the most BB. However, bad habits are hard to break, and baseball did not immediately adopt OBP as an important statistic. We can use data to try to make a case that BB do indeed help produce runs and should be valued.


### Using baseball data 

To illustrate how data can be used to answer questions in baseball, we start with a simple example. We will compare BB to stolen bases (SB). In contrast to BB, total SB were considered important and an award[^moneyball-motivation-8] given to the player with the most. But players with high totals of SB also made more outs, as they did not always succeed. Does a player with a high SB total help produce runs? Can we use data to determine if it's better to pay for players with high BB or high SB?
...
Now, let's start with an obvious question: do teams that hit more HR score more runs? 
...
We know that HR cause runs because when a player hits a HR, they are guaranteed at least one run. Could it be that HR also cause BB and this makes it appear as if BB cause runs? Are BB *confounded* with HR? Linear regression can help us parse out the information and quantify the associations. This, in turn, will aid us in determining what players to recruit. Specifically, we will try to predict things like how many more runs a team will score if we increase the number of BB, but keep the HR fixed.

### Regression applied to baseball statistics

Can we use regression with these data? First, notice that the HR and runs data, shown above, appear to be bivariate normal. Specifically, qqplots confirm that the normal distribution is a useful approximation for the distributions of runs, for each HR strata:
...
The estimated slope is `r round(lm(R ~ HR, data = dat)$coef[[2]], 2)`. This means that, on average, teams that hit one additional home run per game score `r round(lm(R ~ HR, data = dat)$coef[[2]], 2)` more runs per game than teams with the same number of games but fewer home runs. Considering that many games are decided by just one run, this difference can translate into a substantial increase in wins.  Not surprisingly, HR hitters are very expensive. Because the A's were working on a budget, they needed to find some other way to increase wins. In the next section, we examine this more carefully.


## Confounding

Previously, we noted a strong relationship between Runs and BB. If we find the regression line for predicting runs from BB, we get a slope of:
...
Although our understanding of the application tells us that HR cause BB, but not the other way around, we can still check if stratifying by BB makes the effect of HR go down. To do this, we use the same code except that we swap HR and BB. In this case, the slopes do not change much from the original:
...
It is somewhat complex to compute regression lines for each stratum. We are essentially fitting models like this:

$$
\mathrm{E}[R \mid BB = x_1, \, HR = x_2] = \beta_0 + \beta_1(x_2) \cdot x_1 + \beta_2(x_1) \cdot x_2
$$

with the slopes for $x_1$ is a function of $x_2$, $\beta_1(x_2)$, and the other way around. But is there an easier approach?
...
Because the data are approximately normal and the conditional distributions were also normal, we are justified in using a linear model:

$$
Y_i = \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + \varepsilon_i
$$

with $Y_i$ runs per game for team $i$, $x_{i,1}$ BB per game, $x_{i,2}$ the HR per game, and $\varepsilon_i$ assumed to be independent and identically distributed. This fits into our linear model framework, which means we can use `lm` to fit the model.
...
### Modeling team run production {#sec-run-production-model}
...
where  $Y_i$ represents *runs produced per plate appearance*, and  $x_{i1}, x_{i2}, x_{i3}, x_{i4}$ represent BB, singles, extra bases, and HR per plate appearance, respectively.  

We can fit the model to the data before the 2002 season, since we are building a team for 2002 with the information we had before the season started:

```{r}
fit <- filter(dat, yearID < 2002) |> 
  lm(R ~ BB + singles + XB + HR, data = _)
```


We note that the fitted model assigns similar weights to base on balls (BB) and singles:

```{r}
fit$coefficients[-1]
```

This confirms our suspicion that walks are nearly as valuable as singles in contributing to a team’s run production.


### Model diagnostics

Because this model is not derived from any underlying physical law, it is essential to check whether it adequately fits the data and can be used for prediction. We leave a full diagnostic assessment as an exercise, but we demonstrate here one key evaluation: testing whether a model fit to data before 2002 can successfully predict team performance in 2002.

To do this, we use the fitted model to estimate each team's expected number of runs for the 2002 season:

```{r}
R_hat <- dat |> filter(yearID == 2002) |> predict(fit, newdata = _)
```

The results show that the model, trained on data from 1962–2001, predicts 2002 team runs remarkably well:

```{r team-run-predictions}
#| echo: false
filter(dat, yearID == 2002) |> 
...
### A model-based metric for players
...
players <- Batting |> filter(yearID %in% 1999:2001) |> 
  group_by(playerID) |>
  summarize(PA = sum(AB+BB), H = sum(H), HR = sum(HR),  
            XB = sum(X2B + X3B),  singles = H - XB - HR, BB = sum(BB),
            AVG = sum(H)/sum(AB)) |>
  filter(PA >= 100) |>
...
Now that statistics are in per-PA rates, we can use the model fitted to team-level data to predict how many runs per plate appearance each player will produce:

```{r}
players$R_hat <- predict(fit, players)*37.5
```

Because each team has, on average, 37.5 plate appearances a game, we covert the predicted runs per plate appearance to predicted runs per game by multiplying by 37.5. This allows us to compare players' contributions on a per-game basis, which is more intuitive and relevant for team-building decisions.

### Moneyball in practice

In the previous scatterplot showing predicted and observed runs, we see that Oakland, despite operating with one of the lowest payrolls in the league, performed as an above-average offensive team. Here we describe some of the details of how they achieved this.

Having built a model to estimate each player's expected run contribution, we can now apply it to real-world decision-making, just as the Oakland A’s did during the 2002 season. To put our predictions into context, we need to combine the performance metrics with additional information about the players, including their salaries and playing status. This allows us to identify not only who is most productive, but who offers the greatest *value* for the team's limited budget.

To do this, we will merge our player-level performance estimates with data from the `Salary`, `People`, and `Appearances` tables. These datasets include contract information, biographical details, and position data, which together provide the foundation for evaluating and comparing players in a realistic team-building scenario.

We begin by adding the salary each player earned in 2002 and removing players who did not appear in that season:
...
If you followed baseball during the era in question, you will recognize the top run producers and not be surprised that they garnered high salaries:
...
Note that 9 runs a game is almost double the league average!

If we plot predicted runs produced versus salary, we can see that there is substantial variation across players in predicted runs produced and that, not surprisingly, players who produce runs garner higher salaries. 
...
players |> filter(debut <= 1996) |>
  ggplot(aes(salary/10^6, R_hat)) +
...
The plot also highlights four players. Before the 2002 season, Oakland lost their best run producer, Jason Giambi, because it could not compete with the New York Yankees offer of over \$10 million in salary. Johnny Damon, another above-average run producer (the dashed line shows the average), also left as Oakland was not willing to pay the 7.25 million salary the Boston Red Sox offered him. Oakland had to make up for this run production, but with a very limited budget. To do this, their first addition was Scott Hatterberg, whose salary was only $900,000, one of the lowest in the league, yet he was predicted to produce more than average runs. Note that among the players in our data frame, Hatterberg, while having a below league average AVG, ranked near the top 10% in BB per plate appearance. Similarly, David Justice had a near league average AVG, but ranked near the top 5% in terms of BB per plate appearance. These savings permitted the A's to upgrade one of their current players, Frank Menechino, for Ray Durham. 


```{r, echo=-c(1,5)}
local({options(tibble.print_max = Inf, tibble.width = Inf)
t1 <- filter(players, playerID %in% c("damonjo01", "giambja01", "menecfr01"))
t2 <- filter(players, playerID %in% c("justida01", "durhara01", "hattesc01"))
bind_rows(t1, t2) |> select(nameFirst, nameLast, AVG, BB,  R_hat) 
})
```

Despite losing two star players, the A's predicted runs per game (`R_hat`) dropped only slightly and the savings allowed them to acquire pitcher Billy Koch, who helped reduce opponents' scoring.


### Summary

Through the Moneyball example, we've seen how multivariable regression allows us to disentangle relationships among correlated variables and estimate the effect of one factor while holding others constant. This approach permitted us to identify undervalued players who contributed meaningfully to their teams, even when traditional statistics failed to capture their value. More broadly, multivariable regression is a widely used tool for adjusting for confounding and revealing patterns that would otherwise remain hidden in complex, real-world data.

However, as we have discussed, as powerful as regression models are, they only describe associations; they do not prove causation. A statistically significant relationship between two variables does not necessarily mean that one causes the other. The Moneyball story illustrates this well: while data analysis revealed new insights, those conclusions were credible only because they were supported by baseball's underlying mechanisms. 

The A’s success with Moneyball went far beyond recognizing that players with high on-base percentages were undervalued. Their staff collected, cleaned, and analyzed vast amounts of data, not just from professional games, but also from college and minor-league players, to uncover undervalued players. They used statistical models to forecast future performance, data wrangling to organize scattered records into usable formats, and quantitative analysis to guide player recruitment and strategy. Data analysis even informed coaching decisions, such as which pitches to favor or how to adjust batting approaches. What began as an experiment in baseball has since spread across nearly every major sport, transforming how teams make decisions and evaluate performance.

This case study illustrates the power and versatility of multivariable regression. By extending the simple linear model framework, we were able to adjust for confounding variables and estimate the contribution of each factor while holding others constant. The same mathematical ideas introduced earlier, expectation, standard deviation, and conditional distributions, underpin this analysis, and the same `lm` function in R provides the computational machinery to fit these models. In short, the Moneyball example demonstrates how the linear modeling tools we have developed throughout this part of the book can be applied to real-world data to make informed, evidence-based decisions.

## Exercises


1\. We have shown how BB and singles have similar predictive power for scoring runs. Another way to compare the usefulness of these baseball metrics is by assessing their stability across the years. Since we have to pick players based on their previous performances, we prefer metrics that are more stable. In the following exercises, we will compare the stability of singles and BB.

Before we begin, we want to generate two tables. One for 2002 and another for the average of the 1999-2001 seasons. We want to define per-plate appearance statistics. Here is how we create the 2002 table, keeping only players with more than 100 plate appearances:
...
6\. Now let's repeat the above for each year since 1962 and make a plot. Use `summarize` and the **broom** package to fit this model for every year since 1962.

7\. Use the results of the previous exercise to plot the estimated effects of BB on runs.

8\.  Write a function that takes R, HR, and BB as arguments and fits two linear models: `R ~ BB` and `R~BB+HR`. Then use the `summary` function to obtain the `BB` for both models for each year since 1962. Then plot these against each other as a function of time.

9\. Since the 1980s, sabermetricians have used a summary statistic different from batting average to evaluate players. They realized walks were important and that doubles, triples, and HRs should be weighed more than singles. As a result, they proposed the following metric:
