# Estimates and Confidence Intervals {#sec-estimates-and-ci}
...
Although typically the results of polls run by political candidates are kept private, polls are also conducted and made public by news organizations because results tend to be of interest to the general public. We will eventually be looking at these public datasets.
...
The **behdslabs** package includes a function that shows a random draw from this urn:
...
## Populations, samples, parameters, and estimates {#sec-parameters-and-estimates}
...
Can we do this with the 25 observations above? It is certainly informative. For example, given that we see 13 red and 12 blue beads, it seems unlikely that $p$ \> 0.9 or $p$ \< 0.1. But are we ready to predict with certainty that there are more red beads than blue in the jar?
...
To understand how good our estimate is, we will describe the statistical properties of the estimate defined above, the sample proportion $\bar{X}$. Remember that $\bar{X}$ is a random variable. As the sum of independent draws divided by a non-random constant, the rules we covered in @sec-mean-var-eqs apply.
...
We can also use what we learned to determine the standard deviation: 
$$
\mathrm{SD}[\bar{X}] = \sqrt{p(1-p)/N}
$$

The standard deviation of an estimator is called its *standard error* (SE). It quantifies how much the estimate is expected to vary from one sample to another. A smaller standard error indicates a more precise estimate.

We use the notation $\mathrm{SE}[\bar{X}]$ to emphasize that we are referring to the variability of the estimator $\bar{X}$, rather than the variability of the underlying data.
...
The plot shows that we would need a poll of over 10,000 people to achieve a standard error that low. We rarely see polls of this size due in part to the associated costs. According to the Real Clear Politics table, sample sizes in opinion polls range from 500 to 3,500 people. For a sample size of 1,000 and $p=0.51$, the standard error is:
...
If we standardize $\bar{X}$ by subtracting its expected value and dividing by its standard error, we obtain a standard normal random variable $Z$:
...
The same reasoning applies for any desired error. To find the error $\epsilon$ that gives a 95% probability that $\bar{X}$ is within that range of $p$, we solve for \epsilon in the expression:
...
For example, `qnorm(0.975)` gives 1.96 for a 95% confidence interval (the multiplier we used above), and `qnorm(0.995)` gives 2.58 for a 99% interval.
...
In practice, pollsters almost always report 95 percent confidence intervals. Unless otherwise stated, you should assume that the reported margin of error is based on this level of confidence. Results are typically presented as "estimate ± margin of error." For example, if $\bar{X} = 0.52$ and has a margin of error of $0.03$, the result would be reported as "52% ± 3%"
...
9\. If $p=0.45$, it means the Republicans are winning by a relatively large margin, since $\theta = -0.1$, which is a 10% margin of victory. In this case, what is the standard error of $\hat{\theta} = 2\bar{X}-1$ if we take a sample of $N=25$?
...
13\. The vector `errors` contains, for each simulated sample, the difference between our estimate $\bar{X}$ and the actual $p$. We refer to this difference as the *error*. Compute the average and make a histogram of the errors generated in the Monte Carlo simulation,
...
14\. Note that the error $\bar{X}-p$ is a random variable. In practice, the error is not observed because we do not know $p$. Here, we observe it since we constructed the simulation. Using the Monte Carlo simulation, what is the average size of the error if we define the size by taking the absolute value $| \bar{X} - p |$?

15\. The standard error is related to the typical **size** of the error we make when predicting. For mathematical reasons related to the Central Limit Theorem, we actually use the standard deviation of `errors`, rather than the average of the absolute values, to quantify the typical size. Using the Monte Carlo simulation, what is this standard deviation of the errors?

16\. The theory we just learned tells us what this standard deviation is going to be because $\mathrm{SE}[\bar{X}-p]=\mathrm{SE}[\bar{X}]$ and we showed how to compute this. What does theory tell us the standard error $\mathrm{SE}[\bar{X}-p]$ is for a sample size of 100?

17\. In practice, we don't know $p$, but we construct an estimate of $\mathrm{SE}[\bar{X}-p]$ by plugging in $\bar{X}$ for $p$ into the expression from exercise 16. Compute this estimate.
...
21\. To corroborate your answer to exercise 20, make a qqplot of the `errors` you generated in exercise 12 to see if they follow a normal distribution.
...
For the next exercises, we will use actual polls from the 2016 election included in the **behdslabs** package.
